...
Clústeres privados:: Si el clúster es privado, el https://["redes autorizadas"^] Debe permitir la dirección IP del servicio Astra Control:
+
--
52.188.218.166/32
...
.Paso
. Vaya a la https://["Página de Cloud Volumes Service de NetApp"^] En Google Cloud Marketplace, seleccione *Compra* y siga las indicaciones.
+
https://["Siga las instrucciones paso a paso de la documentación de Google Cloud para adquirir y activar el servicio"^].
...
.Paso
. https://["Utilice la consola de Google Cloud o la interfaz de línea de comandos gcloud para habilitar las siguientes API"^]:
+
** Google Kubernetes Engine
** Almacenamiento en cloud
** API JSON para el almacenamiento en cloud
...
.Pasos
. Vaya a Google Cloud y. https://["cree una cuenta de servicio mediante la consola, el comando gcloud u otro método preferido"^].
...
.Pasos
. Vaya a Google Cloud y. https://["cree una clave de cuenta de servicio mediante la consola, el comando gcloud u otro método preferido"^].
...
.Pasos
. https://["Acceda al programa Global Regions Maps de Cloud Central de NetApp"^] E identifique el tipo de servicio que usará en la región de Google Cloud en la que resida su clúster.
+
Cloud Volumes Service ofrece dos tipos de servicios: CVS y CVS-Performance. https://["Obtenga más información sobre estos tipos de servicio"^].

. https://["Vaya a Cloud Volumes en Google Cloud Platform"^].
...
+
Para obtener más detalles sobre el uso de estos comandos, consulte https://["Inicio rápido de Cloud Volumes Service para GCP"^].

+
https://["Obtenga más información sobre cómo configurar el acceso a los servicios privados y la configuración de la conexión a redes"^].
